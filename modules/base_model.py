...
def init_rnn_state(batchsize,num_hiddens):
...
class Transformer(nn.Module):
    ...
    def forward(self, src, tgt, src_mask, tgt_mask, memory_matrix):
        memory, goabl_h = self.encode(src, src_mask)
        return self.decode(memory, goabl_h, src_mask, tgt, tgt_mask, memory_matrix=memory_matrix)

    def encode(self, src, src_mask):
        return self.encoder(self.src_embed(src), src_mask)

    def decode(self, memory, goabl_h, src_mask, tgt, tgt_mask, past=None, memory_matrix=None):
        ...
        return self.decoder(embeddings, memory, goabl_h, src_mask, tgt_mask, past=past)
...
class Encoder(nn.Module):
    def __init__(self, layer, N):
        ...
        self.gru = GRU(layer.size, layer.size)
        self.d_model = layer.size
        self.init_linear()
    ...
    def forward(self, x, mask):
        h, c = init_rnn_state(x.shape[0], x.shape[-1])  # 这里先初始化gru的参数
        goabl = x.mean(-2)
        h, c = self.gru(goabl, h, c)
        for layer in self.layers:
            ...
            h, c = self.gru(goabl, h, c)
            # print(h.unsqueeze(dim=1).shape)
            # print(x.shape)
        # print(x.shape)
        #x = self.wordfusion(x, h)
        # print(x.shape)
        # x = torch.cat([x, h.unsqueeze(dim=1)], dim=1)
        return self.norm(x), self.norm(h)

    def wordfusion(self, x, goabl_h):
        goabl = goabl_h # lstm

        x_cat = torch.cat([x, goabl.unsqueeze(1).expand_as(x)], dim=-1)# b,1,512
#         x_cat = torch.cat([x, goabl_h_t.view(ng, -1, self.size).expand_as(x)], dim=-1)
        x_f = self.fuse_layer(x_cat) + x
        x = self.fuse_layer_norm(x_f) # 16,59,512.
        return x
...
class Decoder(nn.Module):
    ...
    def forward(self, x, memory, goabl_h, src_mask, tgt_mask, past=None):
        if past is not None:
            present = [[], []]
            x = x[:, -1:]
        ...
        for i, (layer, layer_past) in enumerate(zip(self.layers, past)):
            x = layer(i, self.num_layer, x, memory, goabl_h, src_mask, tgt_mask,
                      layer_past)
...
class DecoderLayer(nn.Module):
    def __init__(self, size, self_attn, src_attn, feed_forward, dropout):
        ...
        self.fuse_layer_norm = nn.LayerNorm(size)

    def forward(self, i, n, x, memory, goabl_h, src_mask, tgt_mask, layer_past=None):
        m = memory#[:, :-1, :]
        # index = torch.tensor([1,3]).to("cuda")
        # x = x
        # print(x.shape)
        if layer_past is None:
            if i <= n:
                x = self.wordfusion(x, goabl_h)
                ...
                x = self.sublayer[1](x, lambda x: self.src_attn(x, m, m, src_mask))
                # x = self.wordfusion(x, goabl_h)
                return self.wordfusion(self.sublayer[2](x, self.feed_forward), goabl_h)
        else:
            if i <= n:
                x = self.wordfusion(x, goabl_h)
                present = [None, None]
                ...
                goabl_h = self.norm(self.feed_forward(goabl_h))
                present = [None, None]
                x, present[0] = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, tgt_mask, layer_past[0]))
                x, present[1] = self.sublayer[1](x, lambda x: self.src_attn(x, m, m, src_mask, layer_past[1]))
                # print(x.shape)
                ...
                # x = self.wordfusion(x, goabl_h)
                # return self.sublayer[2](x, self.feed_forward), present
                return self.wordfusion(self.sublayer[2](x, self.feed_forward), goabl_h), present
    ...
    def wordfusion(self, x, goabl_h):
        ...
        return x
class GRU(nn.Module):
    def __init__(self, in_dim, hidden_dim): # 这里两个维度都一样
        super(GRU, self).__init__()
        # c是包含全部上下文的信息，而h经过c与当前x进行计算是最贴切当前状态的信息。同时也作为模型的输出
    ...
    def forward(self, x, h_1, c_1): # c_1是上一个时刻的记忆单元，这个x是每层encoder的输出
        ...
        c_ = torch.tanh(self.cx_linear(x) + self.ch_linear(h_1)) # 候选c
        # 计算4种数据，然后将其按照公式进行组合
        c = f * c_1 + i * c_ # 求出当前时刻的记忆单元，传到下一个时刻，可以理解为，对当前记忆单元和以往记忆单元进行遗忘或选择
        h = o * torch.tanh(c) # 这里是隐藏状态，对应o,o输出的就是隐藏状态，。可以理解为一个残差连接
        return h, c # h和c的维度全程不变，[b，1，c]根据此初始化一下


class MultiThreadMemory(nn.Module): # 与普通多头注意力机制代码的区别是注意力计算部分
...
class MultiHeadedAttention(nn.Module): # 普通多头注意力机制
...
class Basemodel(AttModel):
    ...
    def make_model(self, tgt_vocab, cmn):
        ...
        model = Transformer(
            Encoder(EncoderLayer(self.d_model, c(attn), c(ff), self.dropout), self.num_layers_ed),
            Decoder(DecoderLayer(self.d_model, c(attn), c(attn), c(ff), self.dropout), self.num_layers_ed, self.num_layers),
    ...
    def __init__(self, args, tokenizer):
        super(Basemodel, self).__init__(args, tokenizer)
        self.args = args
        self.num_layers = args.num_layers
        self.num_layers_ed = args.num_layers_ed
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks):
        ...
        memory, goabl_h = self.model.encode(att_feats, att_masks)

        return fc_feats[..., :1], att_feats[..., :1], memory, goabl_h, att_masks
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, goabl_h, state, mask):
        if len(state) == 0:
            ys = it.unsqueeze(1)
            ...
            past = state[1:]
        out, past = self.model.decode(memory, goabl_h, mask, ys, subsequent_mask(ys.size(1)).to(memory.device), past=past,
